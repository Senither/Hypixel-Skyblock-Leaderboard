...
class UpdateTask extends Task {
  ...
  async run(app) {
    ...
    let oldMembers = await app.database.getClient().table('players').where('guild_id', this.guild.id)
    ...
    let memberHistoryUsernames = await app.database
      .getClient()
      .table('players')
  ...
  async updateAveragesForGuild(app) {
    ...
    app.database.update(
      ...
      query => {
        return query.whereNotIn('uuid', this.history.uuids).andWhere('guild_id', this.guild.id)
      }
    )
